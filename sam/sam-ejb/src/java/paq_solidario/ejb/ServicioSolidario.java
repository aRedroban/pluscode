 ...
 */
@Stateless
public class ServicioSolidario {
    ...
    /*
    PROCESOS SISTEMA REPORTES SOLIDARIO INICIO
    **
    *
    */
    public String getClientesExitosos(String tipo, String fechaInicio, String fechaFinal) {
        String sql = "";
        sql = "SELECT \n" +
              "    c.id,\n" +
              ...
              "            primer_nombre,\n" +
              "            ' ',\n" +
              "            segundo_nombre) AS nombres,\n" +
              "    c.estado_gestion,\n" +
              "    s.nombre AS subestado\n" +
              "FROM\n" +
              "    clientes c\n" +
              "        INNER JOIN\n" +
              "    subestado s ON c.subestado_gestion_id = s.id\n" +
              "        AND c.estado_gestion = 'CONTACTADO'\n" +
              "        AND c.subestado_gestion_id = 1"
                + "    AND c.sub_sub_estado = 'VENTA'\n" +
              "        AND c.es_impreso = 0";
        if (tipo.equals("2")){
            sql += " and c.id = -1";
        }
        if (tipo.equals("3")){
            sql += " and fecha_gestion between '"+fechaInicio+" 00:00:00' and '"+fechaFinal+" 23:59:59'";
        }        
        return sql;
    }
...
public void crearInforme(String ruta, Map parametros)
{
  try
  {
...
public void verVisor()
{
  ...
  viewer.setVisible(true);
    ...
    /*
    FIN DE PROCESOS SOLIDARIO MYSQL
    */
